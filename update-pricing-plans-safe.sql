-- Safe Update Pricing Plans and Email Limits
-- This script safely updates the database to support the new pricing structure:
-- Starter: $29/month - 3,000 emails
-- Growth: $49/month - 5,000 emails  
-- Pro: $129/month - 10,000 emails

-- Step 1: Update the plan constraint to include 'starter' plan
ALTER TABLE users DROP CONSTRAINT IF EXISTS users_plan_check;
ALTER TABLE users ADD CONSTRAINT users_plan_check 
  CHECK (plan = ANY (ARRAY['free'::text, 'starter'::text, 'growth'::text, 'pro'::text]));

-- Step 2: Update subscription status constraint if needed
ALTER TABLE users DROP CONSTRAINT IF EXISTS users_subscription_status_check;
ALTER TABLE users ADD CONSTRAINT users_subscription_status_check 
  CHECK (subscription_status = ANY (ARRAY['free'::text, 'active'::text, 'cancelled'::text, 'expired'::text]));

-- Step 3: Safely drop and recreate functions
-- Drop existing functions with different possible signatures
DO $$ 
BEGIN
    -- Drop get_email_limit function if it exists
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'get_email_limit') THEN
        DROP FUNCTION IF EXISTS get_email_limit(TEXT, TEXT);
        DROP FUNCTION IF EXISTS get_email_limit(TEXT);
    END IF;
    
    -- Drop can_send_emails function if it exists
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'can_send_emails') THEN
        DROP FUNCTION IF EXISTS can_send_emails(UUID, INTEGER);
    END IF;
    
    -- Drop increment_email_count function if it exists
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'increment_email_count') THEN
        DROP FUNCTION IF EXISTS increment_email_count(UUID, INTEGER, UUID);
    END IF;
END $$;

-- Step 4: Create the get_email_limit function
...
-- Step 5: Create the can_send_emails function
...
-- Step 6: Create the increment_email_count function
...
-- Step 8: Update any existing users to have proper subscription status
UPDATE users 
...
-- Success message
SELECT 'Database updated successfully! New pricing plans are now supported.' as message;
